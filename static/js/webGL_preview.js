...
function getPosition(index) {
  ...
  var positionY =
    (layouts[scene][i + 1] * 255 + layoutsl[scene][i + 1]) / 65536 - 0.5;
  var positionZ =
    (layouts[scene][i + 2] * 255 + layoutsl[scene][i + 2]) / 65536 - 0.5;
...
function getNColor(index) {
  ...
  var color = [
    layoutsRGB[scene][i],
    layoutsRGB[scene][i + 1],
    layoutsRGB[scene][i + 2],
  ];
  return color;
}
...
function getLink(index) {
  ...
  var link = {};
  link["start"] =
    links[scene][i] + links[scene][i + 1] * 128 + links[scene][i + 2] * 16384;
  link["end"] =
    links[scene][i + 4] +
    links[scene][i + 5] * 128 +
    links[scene][i + 6] * 16384;
