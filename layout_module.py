...
LAYOUT_IDS = ["random", "force-directed", "eigen", "local", "global", "importance", "spectral", "spring"] # ids used in session data
LAYOUT_NAMES = ["Random Layout", "Force-Directed", "EigenUMAPLayout", "cartoGRAPHsLocal", "cartoGRAPHsImportance", "Spectral", "Spring"] # names used for texture generation  !!! not implemented yet !!!
LAYOUT_TABS = ["Random",  "Force-Directed", "Eigenlayout", "cartoGRAPHs Local", "cartoGRAPHs Global", "cartoGRAPHs Importance", "Spectral", "Spring"] # names used in panel display and in connect_socketIO_main.js to switch tabs
...
def layout_eigen(ordered_graph)->dict:
    """
    Eigen Layout Generation Function
    """
...
def layout_forcedirected(ordered_graph)->dict:
    """
    spring layout
    """
    # integrety checks
    ...
    # actual layout to get node positions
    try:
        layout = nx.spring_layout(G=ordered_graph, dim=3, iterations=10)
        positions = []
        for node in ordered_graph.node_order:
            pos = layout[node]
            positions.append([pos[0], pos[1], pos[2]])
